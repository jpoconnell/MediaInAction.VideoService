...
import type { DashboardInput } from '../series-ns/models';
...
export class ToBeMappedService {
  ...
  getDashboard = (dashboardInput: DashboardInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, object>({
      method: 'GET',
      url: '/api/video/to-be-mapped/dashboard',
      params: { filter: dashboardInput.filter },
    },
    { apiName: this.apiName,...config });

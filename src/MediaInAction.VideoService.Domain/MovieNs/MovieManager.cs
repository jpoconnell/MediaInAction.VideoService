...
public class MovieManager : DomainService
{
    ...
    public async Task<Movie> CreateUpdateMovieAsync(MovieCreateDto movieCreateDto)
    {
        if (movieCreateDto.Name != null)
        {
            ...
            {
                var update = 0;
                if (dbMovie.MovieStatus != movieCreateDto.MovieStatus)
                {
                    dbMovie.MovieStatus = movieCreateDto.MovieStatus;
                    update++;
                }
    ...
    public async Task<Movie> UpdateAsync(MovieCreateDto movieCreateDto)
    {
        ...
        if (movieList.Count == 1)
        {
            ...
            if (movie.MovieStatus != movieCreateDto.MovieStatus)
            {
                movie.MovieStatus = movieCreateDto.MovieStatus;
                update++;
            }

...
public class SeriesAppService : VideoServiceAppService, ISeriesAppService
{
    ...
    public Task ImportInActiveAsync()
    {
        throw new NotImplementedException();
    }
    ...
    public async Task ExportInActiveAsync()
    {
        ISpecification<Series> specification = Specifications.SpecificationFactory.Create("a:");
        var seriess =
            await _seriesRepository.GetListPagedAsync(specification, 0,
                10000, "SeriesName" );

        string json = JsonSerializer.Serialize(seriess);
        File.WriteAllText("series.json", json);
    }

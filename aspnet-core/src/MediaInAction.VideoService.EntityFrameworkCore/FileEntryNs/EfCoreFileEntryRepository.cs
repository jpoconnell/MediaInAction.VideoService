...
public class EfCoreFileEntryRepository : EfCoreRepository<VideoServiceDbContext, FileEntry, Guid>, IFileEntryRepository
{
    ...
    public Task<FileEntry> GetByFileEntryNoAsync(int fileEntryNo, 
        bool includeDetails = true, 
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<FileEntry> FindFileEntry(string server, string directory, string fileName, ListType listName)
    {
        try
        {

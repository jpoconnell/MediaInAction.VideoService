...
public class EfCoreToBeMappedRepository : EfCoreRepository<VideoServiceDbContext, ToBeMapped, Guid>, IToBeMappedRepository
{
    ...
    public async Task<List<ToBeMapped>> GetDashboardAsync(
    ...
    public async Task<List<ToBeMapped>> GetMapped(ToBeMappedStatus processed, int limit)
    {
        try
        {
            var dbSet = await GetDbSetAsync();
            return await dbSet
                .Where(e => e.Status == processed )

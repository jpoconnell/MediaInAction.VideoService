...
public class EfCoreMovieRepository : EfCoreRepository<VideoServiceDbContext, Movie, Guid>, IMovieRepository
{
    ...
    public async Task<Movie> GetByMovieNameYear(string movieName, int firstAiredYear)
    {
        try
        {
            var dbSet = await GetDbSetAsync();
            return await dbSet
                .Where(e => e.Name == movieName  && e.FirstAiredYear == firstAiredYear)
                .FirstAsync();
        }
        catch 
        {
    ...
    public async Task<List<Movie>> GetActiveList()
    {
        try
        {
            var dbSet = await GetDbSetAsync();
            return await dbSet
                .Where(e => e.MovieStatus == MovieStatus.Active  )
                .ToListAsync();
        }
        catch 
        {

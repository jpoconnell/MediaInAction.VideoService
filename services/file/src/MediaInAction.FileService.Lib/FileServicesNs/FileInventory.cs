...
using MediaInAction.FileService.Lib.FileMappingsNs;
...
public class FileInventory : IFileInventory
{
    ...
    private readonly IFileMappingLibService _fileMappingLibService;
    ...
    public FileInventory(
        ...
        IFileMappingLibService fileMappingLibService,
        FileServicesConfiguration fileServicesConfiguration)
    {
        ...
        _fileMappingLibService = fileMappingLibService;
    }
    ...
    public async Task SendMappings()
    {
        _logger.LogInformation("SendMappings service started");
         var mappingsList = await _fileMappingLibService.GetUnMapped();
                    ...
                    foreach (var mediaType in mediaTypes)
                    {
                        foreach (var file in Directory.GetFiles(dir, "*." + mediaType))
                        {
                            try
                            {
                                ...
                                var rec = new FileEntryCreateDto()
                                {
                                ...
                                await _fileEntryService.CreateAsync(rec);
                            }
                            catch (Exception ex)
                            {

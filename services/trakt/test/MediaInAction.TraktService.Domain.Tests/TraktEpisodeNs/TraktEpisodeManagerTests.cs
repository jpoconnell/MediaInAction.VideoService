...
namespace MediaInAction.TraktService.TraktEpisodeNs
{
    public class TraktEpisodeManagerTests : TraktServiceDomainTestBase
    {
        ...
        [Fact]
        public async Task Should_Set_Season_Of_Trakt_Episode()
        {
            ...
            {
                var episode = await _traktEpisodeRepository.GetListAsync();
                episode[0].SeasonNum = 1;
                await _traktEpisodeRepository.UpdateAsync(episode[0]);
            });

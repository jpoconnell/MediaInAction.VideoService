    ...
    ITraktShowRepository
{
    ...
    public async Task<TraktShow> FindBySlugAsync(string showSlug, 
        bool includeDetails = true, 
        CancellationToken cancellationToken = default)
    {
        try
    ...
    public async Task<List<TraktShow>> GetListAsync(int skipCount, 
        int maxResultCount, 
        string sorting, 
        string filter = null)
    {
        var queryable = await GetMongoQueryableAsync();
        return await queryable
            .WhereIf<TraktShow, IMongoQueryable<TraktShow>>(
                !filter.IsNullOrWhiteSpace(),
                show => show.Slug.Contains(filter)
            )
            .OrderBy(sorting)
            .As<IMongoQueryable<TraktShow>>()
            .Skip(skipCount)
    ...
    public async Task<TraktShow> FindSlugAsync(string slug)
    {
        try
        {
            var queryable = await GetMongoQueryableAsync();
            return await queryable.FirstOrDefaultAsync(show => show.Slug == slug );
        }
        catch 
        {
            return null;

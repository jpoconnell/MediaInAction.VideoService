...
public class EmbyProcessActivityLog :  IEmbyProcessActivityLog
{
    ...
    public List<EmbyClient.Dotnet.Model.ActivityLogEntry> GetData()
    {
        ...
        var activityLogList = new List<EmbyClient.Dotnet.Model.ActivityLogEntry>();
        try
        {
    ...
    private async Task UpdateDatabase(List<EmbyClient.Dotnet.Model.ActivityLogEntry> activityList)
    {
    ...
    {        
        if (Configuration.Default.BasePath != "http://feederbox2.jpocl.com:8096/emby")
        {
            Configuration.Default.Username = "jerryocl";
            Configuration.Default.ApiKey.Add("api_key", "8ea083c3fdf54a94aecfdac2f4e4742b");
            Configuration.Default.BasePath = "http://feederbox2.jpocl.com:8096/emby";
